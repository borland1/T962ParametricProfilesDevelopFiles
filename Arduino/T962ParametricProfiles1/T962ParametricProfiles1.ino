...
const profile syntechlf_profile = {
...
const profile rampspeed_test_profile = {
...
const profile pid_control_test_profile = {
...
// **************************
profile* ee2 = profiles[profileidx]; 
uint16_t var1 = ee2->parameter[PREHEAT_START_TEMP];
// **************************
...
void setup() {
  ...
  *tmpProfileName = "New Name";
  ...
  ee2->parameter[PREHEAT_START_TEMP] = 40;
  var1 = ee2->parameter[PREHEAT_START_TEMP];
  ...
  // ee2.profileName = "12345678901234567";  // testing size of ee2 written/read to/from EEPROM
  // int ee2currentByteSize = strlen(ee2->profileName) + 26;  // Test after testing EEwrite of ee2 and parmeter "cooldownEndTemp".
  Serial.print("ee2->name: ");
  Serial.println(ee2->name);
  // Serial.print("profiles[curprofile]->name: ");
  // Serial.println(profiles[curprofile]->name);
  Serial.println();
  ...
  ee2->parameter[PREHEAT_START_TEMP] = 56;
  // for (int i = 0; i < NUMPROFILES; i++) {
    ...
    ee2->parameter[PREHEAT_START_TEMP] = 0;  // clear value in RAM previously stored in EEPROM
    ee2->parameter[COOLDOWN_END_TEMP] = 140;  // was 50 before saved to EEPROM
    ...
    Serial.println( ee2->parameter[PREHEAT_START_TEMP] );
    Serial.println();
    ...
    Serial.println( ee2->parameter[COOLDOWN_END_TEMP] );  // should be restored to 50
...
void Reflow_CalculateTempProfile( void ) {
  ...
  soakEndTemp = (ee2->parameter[SOAK_START_TEMP]) + (ee2->parameter[SOAK_HEAT_RATE] * multiplier * ee2->parameter[SOAK_DWELL_TIME] );
  if ( soakEndTemp > ee2->parameter[SOAK_END_TARGET_TEMP] ){
        soakEndTemp = ee2->parameter[SOAK_END_TARGET_TEMP];
  }
  // Calculate Profile Phase End Times ( ETs )
  // Preheat
  endTime.preheat = ( ee2->parameter[SOAK_START_TEMP] - ee2->parameter[PREHEAT_START_TEMP] ) /
      ( ee2->parameter[PREHEAT_HEAT_RATE] * multiplier ) + ee2->parameter[PREHEAT_START_TEMP_DWELL_TIME];  // ***** can't divide by zero if heat rate = zero
  // Soak
  endTime.soak = endTime.preheat + ee2->parameter[SOAK_DWELL_TIME];
  // Reflow
  endTime.reflow = endTime.soak + ( ee2->parameter[REFLOW_SOLDER_MELT_TEMP] - soakEndTemp ) /
    ( ee2->parameter[REFLOW_HEAT_RATE] ) + ee2->parameter[REFLOW_DWELL_TIME_ABOVE_MELT_TEMP];   // ****** can't divide by zero if heat rate = zero
  // Cooldown
  endTime.cooldown = endTime.reflow + ( ee2->parameter[REFLOW_SOLDER_MELT_TEMP] -
    ee2->parameter[COOLDOWN_END_TEMP] ) / ( ee2->parameter[COOLDOWN_RATE] * multiplier );  // ******* can't divide by zero if heat rate = zero
  ...
  for ( int16_t i = 0; i < NUMPROFILETEMPS; ++i ) {  // 0 - 47 temperatures is normal #
    ...
    // Preheat
    if( t <= ee2->parameter[PREHEAT_START_TEMP_DWELL_TIME] )
      temp = ee2->parameter[PREHEAT_START_TEMP];
    else if( t <= endTime.preheat ) {
      temp = previousTemp + ( ee2->parameter[PREHEAT_HEAT_RATE] * multiplier * inv );
      if( temp > ee2->parameter[SOAK_START_TEMP] )
        temp = ee2->parameter[SOAK_START_TEMP];
    }
    // Soak
    else if( t <= endTime.soak ) {
      temp = previousTemp + ( ee2->parameter[SOAK_HEAT_RATE] * multiplier * inv );
      if ( temp > ee2->parameter[SOAK_END_TARGET_TEMP] )
        temp = ee2->parameter[SOAK_END_TARGET_TEMP];
    }
    // Reflow
    else if( t <= endTime.reflow - ( ee2->parameter[REFLOW_OVERSHOOT_MELT_TEMP_DELTA] / ( ee2->parameter[REFLOW_HEAT_RATE] * multiplier )) ){ // ******* can't divide by zero if heat rate = zero
      temp = previousTemp + ( ee2->parameter[REFLOW_HEAT_RATE] * multiplier * inv );
      if( temp > ee2->parameter[REFLOW_SOLDER_MELT_TEMP] + ee2->parameter[REFLOW_OVERSHOOT_MELT_TEMP_DELTA] )
        temp = ee2->parameter[REFLOW_SOLDER_MELT_TEMP] + ee2->parameter[REFLOW_OVERSHOOT_MELT_TEMP_DELTA];
    }
    else if( t <= endTime.reflow ) temp = previousTemp - ( ee2->parameter[REFLOW_HEAT_RATE] * multiplier * inv );
      
    // Cooldown
    else if( t <= endTime.cooldown ) {
      temp = previousTemp - ( ee2->parameter[COOLDOWN_RATE] * multiplier * inv );
      if ( temp < ee2->parameter[COOLDOWN_END_TEMP] )
          temp = ee2->parameter[COOLDOWN_END_TEMP];
    }
