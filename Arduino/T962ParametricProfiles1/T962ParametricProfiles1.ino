...
void Reflow_CalculateTempProfile( void ) {
  ...
  // Preheat
  endTime.preheat = ( (ee2->parameter[PREHEAT_HEAT_RATE] == 0) ? 0 : ( ee2->parameter[SOAK_START_TEMP] - ee2->parameter[PREHEAT_START_TEMP] ) /
      ( ee2->parameter[PREHEAT_HEAT_RATE] * multiplier ) ) + ee2->parameter[PREHEAT_START_TEMP_DWELL_TIME];
  // Soak
  endTime.soak = endTime.preheat + ee2->parameter[SOAK_DWELL_TIME];
  // Reflow
  endTime.reflow = endTime.soak + ( ( ee2->parameter[REFLOW_HEAT_RATE] == 0 ) ? 0 : ( ee2->parameter[REFLOW_SOLDER_MELT_TEMP] - soakEndTemp ) /
    ( ee2->parameter[REFLOW_HEAT_RATE] * multiplier ) ) + ee2->parameter[REFLOW_DWELL_TIME_ABOVE_MELT_TEMP];
  // Cooldown
  endTime.cooldown = endTime.reflow + ( ( ee2->parameter[COOLDOWN_RATE] == 0 ) ? 0 : ( ee2->parameter[REFLOW_SOLDER_MELT_TEMP] -
    ee2->parameter[COOLDOWN_END_TEMP] ) / ( ee2->parameter[COOLDOWN_RATE] * multiplier ) );
  ...
  for ( int16_t i = 0; i < NUMPROFILETEMPS; ++i ) {  // 0 - 47 temperatures is normal #
    ...
    // Reflow
    else if( t <= endTime.reflow - ( ( ee2->parameter[REFLOW_HEAT_RATE] == 0 ) ? 0 : ( ee2->parameter[REFLOW_OVERSHOOT_MELT_TEMP_DELTA] /
      ( ee2->parameter[REFLOW_HEAT_RATE] * multiplier ) ) ) ) {
