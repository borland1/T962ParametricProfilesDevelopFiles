...
void Reflow_CalculateTempProfile( void ) {
  // unsigned int 
  float temp;  // profile incremental temperatures
  float soakEndTemp = 0;
  const float multiplier = 0.1f; // heat rate multiplier
  ...
  soakEndTemp = ((float) ee2.params[SOAK_START_TEMP]) + ( (float) ee2.params[SOAK_HEAT_RATE] * multiplier * (float) ee2.params[SOAK_DWELL_TIME] );
  if ( soakEndTemp > ee2.params[SOAK_END_TARGET_TEMP] ){
        soakEndTemp = (float) ee2.params[SOAK_END_TARGET_TEMP];
  }
  // Calculate Profile Phase End Times ( ETs )
  // Preheat
  endTime.preheat = (float) ee2.params[SOAK_START_TEMP] - ( (ee2.params[PREHEAT_HEAT_RATE] == 0) ? 0 : ( (float) ee2.params[PREHEAT_START_TEMP] /
      ( (float) ee2.params[PREHEAT_HEAT_RATE] * multiplier ) ) ) + (float) ee2.params[PREHEAT_START_TEMP_DWELL_TIME];
  // Soak
  endTime.soak = endTime.preheat + (float) ee2.params[SOAK_DWELL_TIME];
  // Reflow
  endTime.reflow = endTime.soak + ( (ee2.params[REFLOW_HEAT_RATE] == 0 ) ? 0 : ( ( (float) ee2.params[REFLOW_SOLDER_MELT_TEMP] - soakEndTemp ) /
     ( (float) ee2.params[REFLOW_HEAT_RATE] * multiplier ) ) ) + (float) ee2.params[REFLOW_DWELL_TIME_ABOVE_MELT_TEMP];
  // Cooldown
  endTime.cooldown = endTime.reflow + ( ( ee2.params[COOLDOWN_RATE] == 0 ) ? 0 : ( ( (float) ee2.params[REFLOW_SOLDER_MELT_TEMP] -
    (float) ee2.params[COOLDOWN_END_TEMP] ) / ( (float) ee2.params[COOLDOWN_RATE] * multiplier ) ) );
  ...
  for ( int16_t i = 0; i < NUMPROFILETEMPS; ++i ) {  // 0 - 47 temperatures is normal #
    ...
    if( t <= ee2.params[PREHEAT_START_TEMP_DWELL_TIME] )
      temp = (float) ee2.params[PREHEAT_START_TEMP];
    else if( t <= endTime.preheat ) {
      temp = previousTemp + ( (float) ee2.params[PREHEAT_HEAT_RATE] * multiplier * (float) inv );
      if( temp > ee2.params[SOAK_START_TEMP] )
        temp = (float) ee2.params[SOAK_START_TEMP];
    }
    // Soak
    else if( t <= endTime.soak ) {
      temp = previousTemp + ( (float) ee2.params[SOAK_HEAT_RATE] * multiplier * (float) inv );
      if ( temp > ee2.params[SOAK_END_TARGET_TEMP] )
        temp = (float) ee2.params[SOAK_END_TARGET_TEMP];
    }
    // Reflow
    else if( t <= endTime.reflow - ( ( ee2.params[REFLOW_HEAT_RATE] == 0 ) ? 0 : ( (float) ee2.params[REFLOW_OVERSHOOT_MELT_TEMP_DELTA] /
      ( (float) ee2.params[REFLOW_HEAT_RATE] * multiplier ) ) ) ) {
      temp = previousTemp + ( (float) ee2.params[REFLOW_HEAT_RATE] * multiplier * (float) inv );
      if( temp > (float) ee2.params[REFLOW_SOLDER_MELT_TEMP] + (float) ee2.params[REFLOW_OVERSHOOT_MELT_TEMP_DELTA] )
          temp = (float) ee2.params[REFLOW_SOLDER_MELT_TEMP] + (float) ee2.params[REFLOW_OVERSHOOT_MELT_TEMP_DELTA];
    }
    else if( t <= endTime.reflow ) 
		temp = previousTemp - ( (float) ee2.params[REFLOW_HEAT_RATE] * multiplier * (float) inv ); 
    // Cooldown
    else if( t <= endTime.cooldown ) {
      temp = previousTemp - ( (float) ee2.params[COOLDOWN_RATE] * multiplier * (float) inv );
      if ( temp < ee2.params[COOLDOWN_END_TEMP] )
          temp = (float) ee2.params[COOLDOWN_END_TEMP];
    }
    else temp = 0;  // ends profile and places oven in standby mode.

    ee1.temperatures[i] = (uint16_t) ( temp  + 0.5f ); // rounding up from 0.5 remainder
