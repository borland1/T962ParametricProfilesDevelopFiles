...
void Reflow_Init(void) {
	...
	/* Reflow_LoadCustomProfiles(); */ // not needed, replaced with below  // *************
	
	// Check EEPROM for pmagic number for parameter profiles implementation // *************
	#define PMAGIC (('P' << 8) | 'J') // Magic number for profile parameters // *************
	
	unint16_t parameterMagic = 0xffff; // *************
	EEPROM_Read((uint8_t*)&parameterMagic, 0x00, sizeof(parameterMagic)); // *************
	if (parameterMagic != PMAGIC) {  // *************
		printf("\nNo EEPROM profile parameters found, writing defaults."); // *************
		for (int i = 0; i < Reflow_getNumParameterProfiles(); i++) {  // **************
			ee4 = parameterProfiles[i]; // **************
			uint8_t addr = 0; // **************
			switch(i){ // ********************
		...
		ee4 = parameterProfiles[NV_GetConfig[REFLOW_PROFILE]]; // **************
		Reflow_calculateTempProfile(); // ****************
		parameterMagic = PMAGIC; // *************
		EEPROM_Write((uint8_t*)&parameterMagic, 0x00, sizeof(parameterMagic)); // *********
	} else {  // *************
		printf("\nProfile parameters found, reading.");  // *************
		Reflow_readEEprofileParameters(); // *************
		Reflow_calculateTempProfile(); // ****************
	} // *************
