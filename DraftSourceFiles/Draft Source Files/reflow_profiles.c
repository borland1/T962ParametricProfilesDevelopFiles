...
static const paramProfile syntechlf_paramProfile = {  // **************
	"AMTECH SYNTECH-LF  ", {   // Profile Name  // ********
    	...
    	11, // 1.1 after multiplier // Preheat Heat Rate (deg C/sec)  // *************
    	170,                     // Soak Start Temp (deg C)  // ***************
    	220,                    // Soak End Target Temp (deg C)  // ***************
    	75,                    // Soak Dwell Time (sec)  // ***************
    	7,  // 0.7 after multiplier // Soak Heat Rate (deg C/sec)  // **********
    	...
    	13, // 1.3 after multiplier// Cooldown Rate (deg C/sec)  // ************
...
static const paramProfile custom1_paramProfile = {  // ***************
	"CUSTOM #1          ", {	// Profile Name  // *************
    	...
    	0, // 0.1 after multiplier	// Preheat Heat Rate (deg C/sec)  // **********
    	0,                	// Soak Start Temp (deg C)  // ****************
    	0,                	// Soak End Target Temp (deg C)  // **************
    	0,                		// Soak Dwell Time (sec)  // ***************
    	0,  // 0.1 after multiplier	// Soak Heat Rate (deg C/sec)  // **************
    	0,                	// Reflow Solder Melt Temp (deg C)  // **********
    	0,                 		// Reflow Overshoot Melt Temp Delta (deg C)  // ***************
    	0, // 0.1 after multiplier	// Reflow Heat Rate (deg C/sec)  // ************ 
    	0,                 	// Reflow Dwell Time Above Melt Temp (sec)  // *************
    	0, // 0.1 after multiplier	// Cooldown Rate (deg C/sec)  // **********
...
void Reflow_computeTempDatasetFromParams( paramProfile *e4, ramprofile *e3 ) {  // ******************
	...
	// Preheat  // ******************
	preheatET = e4->params[PREHEAT_HEAT_RATE] == 0 ? 0 : ( e4->params[SOAK_START_TEMP] - e4->params[PREHEAT_START_TEMP] ) /  // ******************
		( e4->params[PREHEAT_HEAT_RATE] * paramMenu[PREHEAT_HEAT_RATE].multiplier ) + e4->params[PREHEAT_START_TEMP_DWELL_TIME];  // ******************
	endTime.preheat = (uint16_t) ( preheatET + 0.5f );  // ******************
		// Soak  // ******************
	soakET = endTime.preheat + e4->params[SOAK_DWELL_TIME];  // ******************
	endTime.soak = (uint16_t) ( soakET + 0.5f );  // ******************
	// Reflow  // ******************
	reflowET = endTime.soak + ( e4->params[REFLOW_HEAT_RATE] == 0 ? 0 : ( ( e4->params[REFLOW_SOLDER_MELT_TEMP] - soakEndTemp ) /  // ******************
	( e4->params[REFLOW_HEAT_RATE] * paramMenu[REFLOW_HEAT_RATE].multiplier ) ) ) + e4->params[REFLOW_DWELL_TIME_ABOVE_MELT_TEMP];  // ******************
	endTime.reflow = (uint16_t) ( reflowET + 0.5f );  // ******************
	// Cooldown  // ******************
	cooldownET = endTime.reflow + ( e4->params[COOLDOWN_RATE] == 0 ? 0 : ( ( e4->params[REFLOW_SOLDER_MELT_TEMP] -  // ******************
		e4->params[COOLDOWN_END_TEMP] ) / ( e4->params[COOLDOWN_RATE] * paramMenu[COOLDOWN_RATE].multiplier ) ) );  // ******************
	...
	for ( int16_t i = 0; i < NUMPROFILETEMPS; i++ ) {  // 0 - 47 temperatures is normal #  // ******************
    	...
    	// Reflow  // ******************
    	else if( t <= ( endTime.reflow - ( e4->params[REFLOW_HEAT_RATE] == 0 ? 0 : ( e4->params[REFLOW_OVERSHOOT_MELT_TEMP_DELTA] /  // **************
			( e4->params[REFLOW_HEAT_RATE] * paramMenu[REFLOW_HEAT_RATE].multiplier ) ) ) ) ) {  // ***************************
