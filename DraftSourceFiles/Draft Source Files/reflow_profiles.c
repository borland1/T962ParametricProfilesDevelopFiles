...
// Preheat // **************** // except for rates, multipliers alway 1.0f // *************************
    {"START TEMP      %3.0f`", PREHEAT_START_TEMP, 0, 270, 1.0f}, // *************************
	{"DWELL TIME      %3.0fS", PREHEAT_START_TEMP_DWELL_TIME, 0, 480, 1.0f}, // *************************
	{"HEAT RATE     %1.1f`/S", PREHEAT_HEAT_RATE, 0, 60, 0.1f}, // *************************
// Soak // *************************
    {"START TEMP      %3.0f`", SOAK_START_TEMP, 0, 270, 1.0f}, // *************************
	{"TARGET TEMP     %3.0f`", SOAK_END_TARGET_TEMP, 0, 270, 1.0f}, // *************************
	{"DWELL TIME      %3.0fS", SOAK_DWELL_TIME, 0, 480, 1.0f}, // *************************
    {"HEAT RATE     %1.1f`/S", SOAK_HEAT_RATE, 0, 60, 0.1f}, // *************************
// Reflow	 // *************************
    {"SOLDER MELT     %3.0f`", REFLOW_SOLDER_MELT_TEMP, 0, 270, 1.0f}, // *************************
	{"TEMP DELTA ABOVE\nSOLDER MELT TEMP %2.0f`", REFLOW_OVERSHOOT_MELT_TEMP_DELTA, 0, 50, 1.0f}, // *************************
...
// Cooldown // *************************
    ...
    {"END TEMP        %3.0f`", COOLDOWN_END_TEMP, 0, 270, 1.0f}, // *************************
}; // *************************
...
void Reflow_computeTempDatasetFromParams( paramProfile *e4, ramprofile *e3 ) {  // ******************
	...
	// Preheat  // ******************
	endTime.preheat = ( ( e4->params[PREHEAT_HEAT_RATE] == 0 ) ? 0 : ( ( (float) e4->params[SOAK_START_TEMP] - (float) e4->params[PREHEAT_START_TEMP] ) /  // ******************
		( (float) e4->params[PREHEAT_HEAT_RATE] * paramMenu[PREHEAT_HEAT_RATE].multiplier ) ) ) + (float) e4->params[PREHEAT_START_TEMP_DWELL_TIME];  // ******************
	// Soak  // ******************
	endTime.soak = endTime.preheat + (float) e4->params[SOAK_DWELL_TIME];  // ******************
	// Reflow  // ******************
	endTime.reflow = endTime.soak + ( ( e4->params[REFLOW_HEAT_RATE] == 0 ) ? 0 : ( ( (float) e4->params[REFLOW_SOLDER_MELT_TEMP] - soakEndTemp ) /  // ******************
	( (float) e4->params[REFLOW_HEAT_RATE] * paramMenu[REFLOW_HEAT_RATE].multiplier ) ) ) + (float) e4->params[REFLOW_DWELL_TIME_ABOVE_MELT_TEMP];  // ******************
	// Cooldown  // ******************
	endTime.cooldown = endTime.reflow + ( ( e4->params[COOLDOWN_RATE] == 0 ) ? 0 : ( ( (float) e4->params[REFLOW_SOLDER_MELT_TEMP] -  // ******************
	(float) e4->params[COOLDOWN_END_TEMP] ) / ( (float) e4->params[COOLDOWN_RATE] * paramMenu[COOLDOWN_RATE].multiplier ) ) );  // ******************
...
void Reflow_readEE_profileParams( paramProfile *e4, ramprofile *e3) { // *************
	  uint8_t addr = 0; // ****************
	  // REFLOW_PROFILE num starts at 0 **********
	  uint8_t curProfile = profileidx + 1; // ***********
	  switch( curProfile) { // ********************
...
void Reflow_writeEE_profileParams(paramProfile *e4) { // *************
	  uint8_t addr = 0; // ****************
	  uint8_t curProfile = profileidx + 1; // *********** REFLOW_PROFILE num starts at 0 **********
	  switch(curProfile) { 	// ********************
        case 1: addr = 0x03; break; // **************
        case 2: addr = 0x22; break; // **************
        case 3: addr = 0x42; break; // **************
