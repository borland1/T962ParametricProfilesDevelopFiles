...
void Reflow_computeTempDatasetFromParams( paramProfile *e4, ramprofile *e3 ) {  // ******************
	...
	// Preheat  // ******************
	endTime.preheat = e4->params[SOAK_START_TEMP] - ( (e4->params[PREHEAT_HEAT_RATE] == 0 ) ? 0 : ( e4->params[PREHEAT_START_TEMP] /  // ******************
	...
	// Reflow  // ******************
	endTime.reflow = endTime.soak + ( (e4->params[REFLOW_HEAT_RATE] == 0 ) ? 0 : ( ( e4->params[REFLOW_SOLDER_MELT_TEMP] - soakEndTemp ) /  // ******************
	( e4->params[REFLOW_HEAT_RATE] * paramMenu[REFLOW_HEAT_RATE].multiplier ) ) ) + e4->params[REFLOW_DWELL_TIME_ABOVE_MELT_TEMP];  // ******************
	// Cooldown  // ******************
	endTime.cooldown = endTime.reflow + ( ( e4->params[COOLDOWN_RATE] == 0 ) ? 0 : ( ( e4->params[REFLOW_SOLDER_MELT_TEMP] -  // ******************
	...
	for ( int16_t i = 0; i < NUMPROFILETEMPS; ++i ) {  // 0 - 47 temperatures is normal #  // ******************
    	...
    	e3->temperatures[i] = temp;  // **** With each loop, Saving temperatures dataset to ramprofile // **************
