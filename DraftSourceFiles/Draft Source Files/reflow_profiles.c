...
// enum for Packing sizes for parameters // *********
typedef enum { // ********
    PACK_9BIT, // ********
    PACK_6BIT // *********
} PackSizes; // ***********

// enum for Packing sizes of each parameter // *********
const PackSizes paramPackSize[PROFILE_PARAM_COUNT] = { // ***********
...
/* // Unused with parameter profiles  // ********************************************
...
*/ // Not used with Parameter Profiles // ********************************************
...
// Preheat // **************** // except for rates, multipliers alway 1.0f // *************************
    {"START TEMP      %3.0f`", PREHEAT_START_TEMP, 0, 280, 1.0f}, // *************************
	{"DWELL TIME      %3.0fS", PREHEAT_START_TEMP_DWELL_TIME, 0, 480, 1.0f}, // *************************
	{"HEAT RATE     %1.1f`/S", PREHEAT_HEAT_RATE, 0, 60, 0.1f}, // *************************
// Soak // *************************
    {"START TEMP      %3.0f`", SOAK_START_TEMP, 0, 280, 1.0f}, // *************************
	{"TARGET TEMP     %3.0f`", SOAK_END_TARGET_TEMP, 0, 280, 1.0f}, // *************************
	{"DWELL TIME      %3.0fS", SOAK_DWELL_TIME, 0, 480, 1.0f}, // *************************
    {"HEAT RATE     %1.1f`/S", SOAK_HEAT_RATE, 0, 60, 0.1f}, // *************************
// Reflow	 // *************************
    {"SOLDER MELT     %3.0f`", REFLOW_SOLDER_MELT_TEMP, 0, 280, 1.0f}, // *************************
	{"TEMP DELTA ABOVE\nSOLDER MELT TEMP %2.0f`", REFLOW_OVERSHOOT_MELT_TEMP_DELTA, 0, 40, 1.0f}, // *************************
	{"HEAT RATE     %1.1f`/S", REFLOW_HEAT_RATE, 0, 60, 0.1f}, // *************************
	{"DWELL TIME ABOVE\nMELT TEMP       %3.0fS", REFLOW_DWELL_TIME_ABOVE_MELT_TEMP, 0, 480, 1.0f}, // *************************
// Cooldown // *************************
	{"COOLDOWN RATE %1.1f`/S", COOLDOWN_RATE, 0, 60, 0.1f},  // *************************
    {"END TEMP        %3.0f`", COOLDOWN_END_TEMP, 0, 280, 1.0f}, // *************************
}; // *************************
...
static const paramProfile rampspeed_test_paramProfile = {  // ************
	"RAMP SPEED TEST    ", { 	// Profile Name  // ************
    	50,                 	// Preheat Start Temp  // **********
    	45,                 	// Preheat Start Temp Dwell Time (sec)  // **********
    	60, // 6.0 after multiplier // Preheat Heat Rate (deg C/sec)  // ***********
    	245,                	// Soak Start Temp (deg C)  // *************
    	245,                	// Soak End Target Temp (deg C)  // ***********
    	170,                	// Soak Dwell Time (sec)  // *************
    	60, // 6.0 after multiplier	// Soak Heat Rate (deg C/sec)  // ************
    	50,                 	// Reflow Solder Melt Temp (deg C)  // *************
    	0,                  	// Reflow Overshoot Melt Temp Delta (deg C)  // ************
    	60, // 6.0 after multiplier	// Reflow Heat Rate (deg C/sec)  // ************
    	180,                	// Reflow Dwell Time Above Melt Temp (sec)  // ****************
    	60, // 6.0 after multiplier	// Cooldown Rate (deg C/sec)  // *****************
    	50                  	// Cooldown End Temp (deg C)  // **************
	}   // **********
};  // ***************
...
static const paramProfile custom1_paramProfile = {  // ***************
	"CUSTOM #1          ", {	// Profile Name  // *************
    	5,                 	// Preheat Start Temp  // ****************
    	50,                 	// Preheat Start Temp Dwell Time (sec)  // *************
    	10, // 0.1 after multiplier	// Preheat Heat Rate (deg C/sec)  // **********
    	0,                	// Soak Start Temp (deg C)  // ****************
    	0,                	// Soak End Target Temp (deg C)  // **************
    	0,                		// Soak Dwell Time (sec)  // ***************
    	10,  // 0.1 after multiplier	// Soak Heat Rate (deg C/sec)  // **************
...
ramprofile ee3 = { "TEMPS DATASET      " }; // *****************

// RAM parameter profile 4 // ********************
paramProfile ee4 = { "PROFILE PARAMETERS " };  // ***************

// profile phase end times var for preheat, soak, reflow, cooldown // ********************	
endTimes endTime;  // ********************

// current profile index
static uint8_t profileidx = 0;

/* // Functions not used with parameter profiles // ********************
...
*/ // end- Functions not used with Parameter Profiles // ********************************************
...
int Reflow_SelectProfileIdx(int idx) {
	if (idx < 0) {
		...
		profileidx = (uint8_t) idx;
	}
	if (profileidx != NV_GetConfig(REFLOW_PROFILE))  // Save to NV only if changed // *************
		NV_SetConfig(REFLOW_PROFILE, profileidx);	
	return profileidx;
}

/* // Functions not used with parameter profiles // ********************
int Reflow_SelectEEProfileIdx(int idx) {  // this function not used with parameter profiles // *************
	if (idx == 1) {
...
int Reflow_GetEEProfileIdx(void) {  // this function not used with parameter profiles // *************
...
void Reflow_ListProfiles(void) {
	for (int i = 0; i < NUMPROFILES; i++) {
		...
		uint8_t namesBuff[PROFILE_NAME_LEN] = {"0"};  // ***************
		EEPROM_Read( namesBuff, (uint32_t) addr, (uint32_t) sizeof(namesBuff) ); // **********
		char names[PROFILE_NAME_LEN] = {"0"}; // ***************
		strcpy(names, (char*) namesBuff); // ensure null-terminated // ***************
		printf("%d: %s\n", (i), names); // ***************
	}
}

char* Reflow_GetProfileName( ramprofile *e3) {  // ***************
	return e3->name; // return name from RAM-based dataset // ******************
}

void Reflow_SetProfileName(char* buff, paramProfile *e4, ramprofile *e3) {  // ***************
	// update RAM-based dataset and EEPROM storage // ******************
	strncpy(e3->name, buff, PROFILE_NAME_LEN); // copy new name to RAM-based dataset // ******************
	strncpy(e4->name, buff, PROFILE_NAME_LEN); // copy new name to RAM-based dataset // ******************
	Reflow_writeEE_profileParams( e4 ); // write new name to EEPROM storage // ******************
}
...
uint16_t Reflow_GetSetpointAtIdx(uint8_t idx) {
	...
	return (uint16_t) ee3.temperatures[idx]; // return temp at index from RAM-based dataset // ******************
}
...
void Reflow_PlotProfile(int highlight) {
	...
	for(int x = 1; x < NUMPROFILETEMPS; x++) {
			...
			if( y == 0 ) {  // ******************
				y = 1;  // plot zero temps at 1 pixel avove x-axis scale // ******************
			} // **********************
			y = YAXIS - y;
...
void Reflow_computeTempDatasetFromParams( paramProfile *e4, ramprofile *e3 ) {  // ******************
	// Copy profile name to RAM-based dataset // ******************
	strncpy(e3->name, e4->name, PROFILE_NAME_LEN); // copy name from *e4 to *e3 RAM-based dataset // ******************
	// Compute profile temperatures from parameters // ******************
	// e4 is pointer to parameter profile in RAM // ******************
	// e3 is pointer to RAM-based temperature dataset // ******************
	// endTime is global struct for phase end times // ******************
	float temp;  // temporary value holder for temperature logic  // ******************
	float soakEndTemp = 0;  // ******************
	float preheatET, soakET, reflowET, cooldownET;  // ******************
	...
	soakEndTemp = ( (float) e4->params[SOAK_START_TEMP]) + ( (float) e4->params[SOAK_HEAT_RATE] * paramMenu[SOAK_HEAT_RATE].multiplier * (float) e4->params[SOAK_DWELL_TIME] );  // ******************
	if ( soakEndTemp > (float) e4->params[SOAK_END_TARGET_TEMP] ) {  // ******************
		soakEndTemp = (float) e4->params[SOAK_END_TARGET_TEMP];  // ******************
	}
	// Compute Reflow Phase End Times  // ******************
	// Preheat  // ******************
	preheatET = ( e4->params[PREHEAT_HEAT_RATE] == 0 ? 0 : ( ( (float) e4->params[SOAK_START_TEMP] - (float) e4->params[PREHEAT_START_TEMP] ) /  // ******************
		( (float) e4->params[PREHEAT_HEAT_RATE] * paramMenu[PREHEAT_HEAT_RATE].multiplier ) ) ) + (float) e4->params[PREHEAT_START_TEMP_DWELL_TIME];  // ******************
	endTime.preheat = (uint16_t) ( preheatET + 0.5f );  // ******************
		// Soak  // ******************
	soakET = preheatET + e4->params[SOAK_DWELL_TIME];  // ******************
	endTime.soak = (uint16_t) ( soakET + 0.5f );  // ******************
	// Reflow  // ******************
	reflowET = soakET + ( e4->params[REFLOW_HEAT_RATE] == 0 ? 0 : ( ( ( e4->params[REFLOW_SOLDER_MELT_TEMP] - soakEndTemp ) /  // ******************
	( (float) e4->params[REFLOW_HEAT_RATE] * paramMenu[REFLOW_HEAT_RATE].multiplier ) ) ) + e4->params[REFLOW_DWELL_TIME_ABOVE_MELT_TEMP] );  // ******************
	endTime.reflow = (uint16_t) ( reflowET + 0.5f );  // ******************
	// Cooldown  // ******************
	cooldownET = reflowET + ( e4->params[COOLDOWN_RATE] == 0 ? 0 : ( e4->params[REFLOW_SOLDER_MELT_TEMP] -  // ******************
		e4->params[COOLDOWN_END_TEMP] ) / ( (float) e4->params[COOLDOWN_RATE] * paramMenu[COOLDOWN_RATE].multiplier ) );  // ******************
	endTime.cooldown = (uint16_t) ( cooldownET + 0.5f );  // ******************
	...
	for ( int16_t i = 0; i < NUMPROFILETEMPS; ++i ) {  // 0 - 47 temperatures is normal #  // ******************
		...
		if( t <= e4->params[PREHEAT_START_TEMP_DWELL_TIME] )  // ******************
      		temp = (float) e4->params[PREHEAT_START_TEMP];  // ******************
    	else if( t <= endTime.preheat ) {  // ******************
    		temp = previousTemp + ( (float) e4->params[PREHEAT_HEAT_RATE] * paramMenu[PREHEAT_HEAT_RATE].multiplier * (float) inv ); // ******************
    		if( temp > e4->params[SOAK_START_TEMP] )  // ******************
    			temp = (float) e4->params[SOAK_START_TEMP];  // ******************
    	}  // ******************
    	// Soak  // ******************
    	else if( t <= endTime.soak ) {  // ******************
    		temp = previousTemp + ( (float) e4->params[SOAK_HEAT_RATE] * paramMenu[SOAK_HEAT_RATE].multiplier * (float) inv ); // ******************
      		if( temp > e4->params[SOAK_END_TARGET_TEMP] )  // ******************
				temp = (float) e4->params[SOAK_END_TARGET_TEMP];  // ******************
    	}  	// ******************
    	// Reflow  // ******************
    	else if( t <= endTime.reflow - ( ( e4->params[REFLOW_HEAT_RATE] == 0 ) ? 0 : ( (float) e4->params[REFLOW_OVERSHOOT_MELT_TEMP_DELTA] /  // **************
			( (float) e4->params[REFLOW_HEAT_RATE] * paramMenu[REFLOW_HEAT_RATE].multiplier ) ) ) ) {  // ***************************
			temp = previousTemp + ( (float) e4->params[REFLOW_HEAT_RATE] * paramMenu[REFLOW_HEAT_RATE].multiplier * (float) inv );   // ******************
			if( temp > (float) e4->params[REFLOW_SOLDER_MELT_TEMP] + (float) e4->params[REFLOW_OVERSHOOT_MELT_TEMP_DELTA] )  // ******************
				temp = (float) e4->params[REFLOW_SOLDER_MELT_TEMP] + (float) e4->params[REFLOW_OVERSHOOT_MELT_TEMP_DELTA];  // ******************
    	}  // ******************
    	else if( t <= endTime.reflow )  // **********************8
			temp = previousTemp - ( (float) e4->params[REFLOW_HEAT_RATE] * paramMenu[REFLOW_HEAT_RATE].multiplier * (float) inv );  // ******************
    	// Cooldown  // ******************
    	else if( t <= endTime.cooldown ) {  // ******************
    		temp = previousTemp - ( (float) e4->params[COOLDOWN_RATE] * paramMenu[COOLDOWN_RATE].multiplier * (float) inv );  // ******************
      		if ( temp < e4->params[COOLDOWN_END_TEMP] )  // ******************
        		temp = (float) e4->params[COOLDOWN_END_TEMP];  // ******************
    	}  	// ******************
    	else temp = 0;  // ends profile and places oven in standby mode.  // ******************

    	e3->temperatures[i] = (uint16_t) ( temp + 0.5f );  // **** With each loop, Saving temperatures dataset to ramprofile // **************
...
void Reflow_pack_profile(const paramProfile *p_profile, uint8_t *buffer) { // **************
    ...
    for (int i = 0; i < PROFILE_PARAM_COUNT; i++) {  // ******************
        ...
        uint8_t bit_len = (paramPackSize[i] == PACK_9BIT) ? 9 : 6; // ******************
