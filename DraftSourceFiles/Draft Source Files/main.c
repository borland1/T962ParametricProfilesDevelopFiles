...
static int32_t Main_Work(void) {
	...
	if (mode == MAIN_SETUP) {
		...
		for (int i = line2Idx; i < line2Idx + 6; i++) { // *************************
			if ((line2Idx == 2 && i == line2Idx + 3) || // ******************
				(line2Idx == 3 && i == line2Idx + 2) || // ******************
				(line2Idx == 1 && i == line2Idx + 1))   // ******************
					 { // *****************************
				LCD_disp_str((uint8_t*)"-- SOAK --", 4 + 6, LCD_ALIGN_CENTER(4 + 6), y, FONT6X6);  // ***************
				y+=7; // ****************************
			}  // ******************************
				...
				LCD_disp_str((uint8_t*)"-- COOLDOWN --", 7 + 6 , LCD_ALIGN_CENTER(7 + 6), y, FONT6X6);  // ***************
				y+=7; // ****************************
				++i; // ***************	
			} // ****************************
		...
		for ( int i = 0; i < (PROFILE_NAME_LEN); i++) {  // *******************
			...
			for (int j = 0; j < 7 && cursorPos == i; j++) LCD_SetPixel( 6 + x + (i * 6), y + j);  // ***************
		}  // *******************************************
