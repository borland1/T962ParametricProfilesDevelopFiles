	...
	if (mode == MAIN_SETUP) {
		...
		for (int i = line2Idx; i < line2Idx + 6; i++) {  // *************************
			...
			if (cursorPos == 8 || cursorPos == 10) {  // for two line menu params // *************
				...
				LCD_disp_str((uint8_t*)buf, count , 1, y, FONT6X6 | (i == (cursorPos - 2 + cursorOffset)) ? INVERT : 0); // **********
				y +=7; // ****************************
				++i; // **********************
				++cursorOffset; // ***************8
				count +=2; // set count for after '\n' // *************
				LCD_disp_str((uint8_t*)buf + count, len - count, 2, y, FONT6X6 | (i == (cursorPos - 2 + cursorOffset)) ? INVERT : 0); // ***********
				y +=7; // ****************************
			} // *************************
			else { LCD_disp_str((uint8_t*)buf, len, 1, y, FONT6X6 | (i == (cursorPos - 2 + cursorOffset)) ? INVERT : 0); } // ************
			y+=7; // ****************************
		}  // **********************
