...
static int32_t Main_Work(void) {
	...
	if (mode == MAIN_SETUP) {
		...
		if (keyspressed & KEY_F1) {	// Previous, vertical scrolling // *************
			...
			if (cursorPos == 2 && line2Idx == 6) { // *****************
				line1Idx -= 4;  // ***************
				line2Idx -= 2;  // *************** 
			}  // ***************
			if (cursorPos == 4 && line2Idx == 8) { // ***************
				line2Idx -= 2;  // *************** 
			}  // ***************
			if (cursorPos == 6 && line2Idx == 11) { // *****************
				line1Idx -= 5;  // ***************
				line2Idx -= 2;  // *************** 
			}  // ***************
			if (cursorPos == 8 && line2Idx == 14) { // ***************
				line2Idx -= 3;  // *************** 					
			}  // ****************
		...
		if (keyspressed & KEY_F2) {	// Next, vertical scrolling // *************
			...
			if (cursorPos == 7 && line2Idx == 4) { // *****************
				line1Idx += 4;  // ***************
				line2Idx += 2;  // *************** 
			}  // ***************
			if (cursorPos == 8 && line2Idx == 6) { // ***************
				line2Idx += 2;  // *************** 
			}  // ***************
			if (cursorPos == 10 && line2Idx == 9) { // *****************
				line1Idx += 5;  // ***************
				line2Idx += 2;  // *************** 
			}  // ***************
			if (cursorPos == 11 && line2Idx == 11) { // ***************
				line2Idx += 3;  // *************** 					
			}  // ****************
		...
		for (int i = line2Idx; i < line2Idx + 6; i++) { // *************************
			...
			if (cursorPos == 8 || cursorPos == 10) {  // for two line menu params // *************
				int count = 0; // *************
				for (int j = 0; buf[j] != '\n'; j++) {  // find count before'\n' *************
					if (buf[j] != '\n') count++;  // *************
				} // ****************************
				LCD_disp_str((uint8_t*)buf, count , 1, y, FONT6X6 | ((cursorPos + 1) == i) ? INVERT : 0); // **********
				++i; // ***************
				y +=7; // ****************************
				count +=3; // set count for after '\n' // *************
				LCD_disp_str((uint8_t*)buf + count, len - count, 2, y, FONT6X6 | ((cursorPos +1) == i) ? INVERT : 0); // ***********
				y +=7; // ****************************
				++i; // ***************
			} // *************************
			LCD_disp_str((uint8_t*)buf, len, 1, y, FONT6X6 | ((cursorPos + 1) == i) ? INVERT : 0); // ************
			y+=7; // ****************************
		}  // **********************
		
		// plot vertical scroll bar graphic // *******************************
		LCD_BMPDisplay(scrollBarTicksbmp, 128, 7 ); // *************
		LCD_BMPDisplay(scrollBarbmp, 128, 9 + ((cursorPos + 1) * 4) ); // *************
