...
static int32_t Main_Work(void) {
	...
	/* / *******************************
		...
		static uint8_t selected = 0;  // *************************
		static uint8_t pageCnt = 1; // Page counter, 1-4 // *********************
		int y = 0;	// *************************
		...
		if (selected >= 0 && selected < 3) { pageCnt = 1; startItem = 0; numItems = 3; }  // PREHEAT, three (0-2) editable parameters // **********
		if (selected > 2 && selected < 7) { pageCnt = 2; startItem = 3; numItems = 4; }   // SOAK, four (3-6) editable parameters // ***********
		if (selected > 6 && selected < 10) { pageCnt = 3; startItem = 7; numItems = 3; }  // REFLOW, four (7-10) editable parameters // *********
		if (selected > 9 && selected < 13) { pageCnt = 4; startItem = 10; numItems = 3; } // COOLDOWN, two (11-12) editable parameters // *********
			
		if (keyspressed & KEY_F1) {	// Prev parameter or loop back // *************
			if (selected > 1) {  // **************************
		    	selected--;  // *****************************
				if (selected == 9) pageCnt--; break; // ****************
				if (selected == 7) pageCnt--; break; // ****************
				if (selected == 5) pageCnt--; break; // ****************
			} else {  // ***************************
				selected < (reflow_getNumParamMenuItems() - 1) ;  // ***********************
				pageCnt = 4; // Reset to last page // ****************
			}  // ***********************
		} // **************************
		if (keyspressed & KEY_F2) { // Next parameter or loop back  // ******************
			if (selected < (reflow_getNumParamMenuItems() - 1) ) {  // *************************
				selected++;  // ************************
				if (selected == 4) pageCnt++; break; // ****************
				if (selected == 8) pageCnt++; break; // ****************
				if (selected == 10) pageCnt++; // ****************
			} else { // ******************************
				selected = 1; // *****************************
				pageCnt = 1; // Reset to first page // ****************
			}	 // *********************************
			}  // *******************************************
		if (keyspressed & KEY_F3) {  // *******************************
				Reflow_editParamMenu_decreaseValue(selected, keyrepeataccel); // ****************
			}  // *********************************************
		if (keyspressed & KEY_F4) {  // **************************
				Reflow_editParamMenu_increaseValue(selected, keyrepeataccel);  // ****************
			}  // ************************************* 
		...
		switch (pageCnt) { // ******************************
			case 1:  // *********************************
					LCD_disp_str((uint8_t*)"PREHEAT", 7, 0, 8 * 0, FONT6X6);  // PREHEAT  // ***************
					LCD_disp_str((uint8_t*)"PG 1 OF 4", 9, 15, 8 * 0, FONT6X6); // ************************
					for (int i = startItem; i < startItem + numItems; i++) { // ***************************
						len = Reflow_editParamMenu_snprintFormattedValue(buf, sizeof(buf), i);  // **********************
						LCD_disp_str((uint8_t*)buf, len, 5, y, FONT6X6 | (selected == i) ? INVERT : 0); // ************
						if (i == 1) { // ************************************
						y +=7; // ****************************
						LCD_disp_str((uint8_t*)"DWELL TIME(S)", 13, 0, y, FONT6X6| (selected == i) ? INVERT : 0); // **********************
						}  // **********************
						y += 7; // *****************************
				  	} // **************************
				  	break; // *****************************
			case 2:  // *******************************
					LCD_disp_str((uint8_t*)"SOAK", 4, 0, 8 * 0, FONT6X6);     // SOAK // **************
					LCD_disp_str((uint8_t*)"PG 2 OF 4", 9, 15, 8 * 0, FONT6X6); // *********************
					for (int i = startItem; i < startItem + numItems; i++) { // *******************
				    	len = Reflow_editParamMenu_snprintFormattedValue(buf, sizeof(buf), i);  // **********************
				    	LCD_disp_str((uint8_t*)buf, len, 5, y, FONT6X6 | (selected == i) ? INVERT : 0); // ************
						if (i == 4) { // *****************************
							y +=7; // *******************************8
							LCD_disp_str((uint8_t*)"TARGET(~)", 13, 0, y, FONT6X6| (selected == i) ? INVERT : 0); // **********************
						}  // ************************
					y += 7; // ******************************
				  	} // **************************
				  	break; // ************************
			case 3: // *****************************
					LCD_disp_str((uint8_t*)"REFLOW", 6, 0, 8 * 0, FONT6X6);	 // REFLOW // ***********
					LCD_disp_str((uint8_t*)"PG 3 OF 4", 9, 15, 8 * 0, FONT6X6); // **************
					for (int i = startItem; i <= startItem + numItems; i++) { // **********************
				    	len = Reflow_editParamMenu_snprintFormattedValue(buf, sizeof(buf), i);  // **********************
				    	LCD_disp_str((uint8_t*)buf, len, 5, y, FONT6X6 | (selected == i) ? INVERT : 0); // ************
						if (i == 5) {  // ************************
							y +=7; // ************************
							LCD_disp_str((uint8_t*)"TEMP(~)", 7, 0, y, FONT6X6| (selected == i) ? INVERT : 0); // **********************
						} // ************************
						if (i == 6) { // ************************
							y +=7;  // ************************
							LCD_disp_str((uint8_t*)"DELTA (~)", 9, 0, y, FONT6X6| (selected == i) ? INVERT : 0); // **********************
						}  // ************************
						y += 7; // ***************************
				  	} // ***********************
				  	break; // ***********************
			case 4: // *****************************
					LCD_disp_str((uint8_t*)"REFLOW", 6, 0, 8 * 0, FONT6X6); // COOLDOWN // *************
			    	LCD_disp_str((uint8_t*)"PG 4 OF 4", 9, 15, 8 * 9, FONT6X6); // **********************
					for (int i = startItem; i <= startItem + numItems; i++) { // ***************************
				    	len = Reflow_editParamMenu_snprintFormattedValue(buf, sizeof(buf), i);  // **********************
				    	LCD_disp_str((uint8_t*)buf, len, 5, y, FONT6X6 | (selected == i) ? INVERT : 0); // ************
						if (i == 10) {  // ************************
							y +=7;  // ************************
							LCD_disp_str((uint8_t*)"ABOVE MELT(S)", 13, 0, y, FONT6X6| (selected == i) ? INVERT : 0); // **********************
						}  // ************************
						y += 7; // *************************
				  	} // ********************
				  	LCD_disp_str((uint8_t*)"COOLDOWN", 8, 0, 8 * 4, FONT6X6); // COOLDOWN // *************
				  	for (int i = 2; i <= numItems; i++) { // **********************
				    	len = Reflow_editParamMenu_snprintFormattedValue(buf, sizeof(buf), i + startItem);  // **********************
				    	LCD_disp_str((uint8_t*)buf, len, 5, y, FONT6X6 | (selected == i) ? INVERT : 0); // ************
						y += 7; // *************************
				  	}  // ************************
			} // *********************************
			...
			if (keyspressed & KEY_S) {  // ******************************
				// Save changes to EEPROM, update profile plot  // ************
				editProfilePageIdx = 0;  // ************************************
			}  // *********************************	
		} // ********************************
	}  // **************************************
	*/  // ****************************************
	//*  // ****************************************
		...
		static int selected = 0; // Profile Name Cursor Position // *********************
		static char pfNameBuf[PROFILE_NAME_LEN];   // **********************
		static char c; // Current character at cursor position // *********************
		static uint8_t init_static_pfNameBuf_c = 1;  // workaround declare static var // *********************
		if (init_static_pfNameBuf_c) { // Init buffer with spaces // *********************
			strncpy( Reflow_GetProfileName(&ee3), pfNameBuf, PROFILE_NAME_LEN); // Profile Name Buffer // *********************
			c = pfNameBuf[0]; // Current character at cursor position // *********************
			init_static_pfNameBuf_c = 0; // Init only once // *********************
		} // *******************************
		...
		for ( int i = 0; i < PROFILE_NAME_LEN; i++) {  // *******************
			LCD_disp_str((uint8_t*)&pfNameBuf[i], 1, x + i, y, FONT6X6 | (selected != i) ? INVERT : 0); // ***********
		}  // *******************************************
		...
		if (keyspressed & KEY_F1) {	// move cursor to left or loop back  // *************
			if (selected >= 0) {  // ******************************************
				selected = (selected - 1) - keyrepeataccel;  // Move to next space left // ******************************
			} else {  // ***************************
				selected = PROFILE_NAME_LEN; // (wrap) // ***********************
			}  // ***************************
			c = pfNameBuf[selected];  // Load current character at cursor position // *********************
		}  // *************************************
		if (keyspressed & KEY_F2) { // move cursor to right or loop back  // ***************
			if (selected <= PROFILE_NAME_LEN) {  // *******************
					selected = (selected + 1) + keyrepeataccel;  // Move to next space right // ********************************************
			} else {  // **********************************************
				selected = 0;  // (wrap) // ********************************
			} // ********************************************
			c = pfNameBuf[selected];  // Load current character at cursor position // *********************
		...
		if (keyspressed & KEY_F3) {  // ASCII values 32-96, & 123-126 are only available fonts  // *************************
			...
			c = (c - 1) - keyrepeataccel; // Decrease ASCII decimal value // *********************
			if ( c < 32 ) // **** (wrap) ******************************
			  pfNameBuf[selected] = 126 + 32 - c; // ****************
			else if ( c > 96 && c < 123 )  // ** (skip) **************************
				  pfNameBuf[selected] = c - 26; // **********************
				else pfNameBuf[selected] =  c; // **********************
			c = pfNameBuf[selected];  // *******************
		}  // ***************************************
		if (keyspressed & KEY_F4) {  // 32-96, & 123-126 are valid ASCII character values // **************************************
			// Increase char ASCII decimal value   // *********************
			c = (c + 1) + keyrepeataccel; // Increase ASCII decimal value // *********************
			if (c > 126 ) // ** (wrap) ******************************
				pfNameBuf[selected] = c + 32 - 127; // *******************
			else if (c > 96 && c < 123)  // *** (skip) ************
				pfNameBuf[selected] = c + 26; // ************* 
			else pfNameBuf[selected] = c; // **********************
			c = pfNameBuf[selected];  // *******************
		...
		if (keyspressed & KEY_S) {  // same renamed profile to RAM and EEPROM // ****************************
			Reflow_SetProfileName( pfNameBuf, &ee4, &ee3 ); // sets in RAM and EEPROM // **************************
			selected = 0; // **************************************
			mode = MAIN_EDIT_PROFILE; // *****************************
		}   // **************************************
	}  // ***************************************
