...
static int32_t Main_Work(void) {
	...
	static int current_edit_profile;
	...
	/* // ************************************
		...
		LCD_disp_str((uint8_t*)"EDIT PROFILE NAME", 17, 14, 7 * 4, FONT6X6); // ********
		...
		static uint8_t selected = 0; // Profile Name Cursor Position // *********************
		static char pfNameBuf[18]; // Profile Name Buffer, 18 chars + null // *********************
		...
		static int numChrs = 18; // **** max size of parameterProfile->name is 19, plus one null char (/n) ****************
		strcpy(pfNameBuf, ee4->name ); // ************************
		...
		for ( int i = 0; i < numChrs; i++) {  // *******************
			char t_buf = pfNameBuf[i]; // **************************
			LCD_disp_str((uint8_t*)t_buf, 1, x + i, y, FONT6X6 | (selected != i) ? INVERT : 0); // ***********
		}  // *******************************************
		...
		if (keyspressed & KEY_F1) {	// move cursor to left or loop back  // *************
			if (selected >= 0) {  // ******************************************
				selected--;  // Move to next space left // ******************************
			} else {  // ***************************
				selected = numChrs; // (wrap) // ***********************
			  }  // ***************************8
		}  // *************************************
		if (keyspressed & KEY_F2) { // move cursor to right or loop back  // ***************
			if (selected <= numChrs) {  // *******************
					selected++;  // Move to next space right // ********************************************
			} else {  // **********************************************
				selected = 0;  // (wrap) // ********************************
			} // ********************************************
		}  // *******************************************
		if (keyspressed & KEY_F3) {  // 32-96, & 123-126 are valid ASCII character values  // *************************
			// Decrease char ASCII decimal value   // *********************
			char c = pfNameBuf[selected] - keyrepeataccel;  // *******************
			if ( c < 32 ) // **** (wrap) ******************************
		...
		if (keyspressed & KEY_S) {  // ****************************
			// Reflow_SetProfileName( (char*) &buf2 ); // **************************				
			// also need to save profileName changes to EEPROM  // *****************************
			selected = 0; // **************************************
			editProfilePageIdx = 0; // ***************************
		}   // **************************************
	}  // ***************************************
		...
		if (keyspressed & KEY_F1) {  // CONFIRM RESTORE  // ************************
			...
			CalculatReflow_calculateTempProfile();
			Reflow_writeEEprofileParams();  // ************
			mode = MAIN_EDIT_PROFILE;  // ****************************
		}  // *********************************
