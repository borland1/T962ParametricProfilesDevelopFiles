...
static int32_t Main_Work(void) {
	...
	if (mode == MAIN_SETUP) {
		...
		if (keyspressed & KEY_F1) {	// Previous, vertical scrolling // *************
			...
			else if (cursorPos < 0) { // wrap around // ****************
				cursorPos = 12;  // *****************
				line1Idx = 10;  // *********************
				line2Idx = 14;  // *******************
			}    // ******************
		...
		if (keyspressed & KEY_F2) {	// Next, vertical scrolling // *************
			...
			else if (cursorPos > 12) { // wrap around // ****************
				cursorPos = 0;  // *****************
				line1Idx = 1;  // *********************
				line2Idx = 2;  // *******************
			}    // ******************
		...
		for (int i = line2Idx; i < line2Idx + 6; i++) {  // *************************
			...
			if (cursorPos == 8 || cursorPos == 10) {  // for two line menu params // *************
				...
				LCD_disp_str((uint8_t*)buf, count , 1, y, FONT6X6 | ((i == cursorPos + 1)) ? INVERT : 0); // **********
				y +=7; // ****************************
				++i; // **********************
				count +=2; // set count for after '\n' // *************
				LCD_disp_str((uint8_t*)buf + count, len - count, 2, y, FONT6X6 | (i == (cursorPos + 1)) ? INVERT : 0); // ***********
				y +=7; // ****************************
			} // *************************
			LCD_disp_str((uint8_t*)buf, len, 1, y, FONT6X6 | (i == (cursorPos + 1)) ? INVERT : 0); // ************
			y+=7; // ****************************
		}  // **********************
