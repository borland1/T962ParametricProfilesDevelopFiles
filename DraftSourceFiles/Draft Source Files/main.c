...
static int32_t Main_Work(void) {
	...
	if (mode == MAIN_SETUP) {
		...
		if (keyspressed & KEY_F1) {	// Previous, vertical scrolling // *************
			--cursorPos;  // *********************
			// if(cursorPos == 2 || cursorPos == 6 || cursorPos == 7 ||  // *******************
			//	 cursorPos == 9 || cursorPos == 10)  cursorOffset += 2;  //  ******************
		...
		if (line1Idx == 1 )       // ******************************
			LCD_disp_str((uint8_t*)"PREHEAT", 7, 40, y, FONT6X6);  // ***************
		else if (line1Idx == 5)   // ***************************
			LCD_disp_str((uint8_t*)"SOAK", 4, 40, y, FONT6X6);     // ***************
		else if (line1Idx == 10)  // ***************************
			LCD_disp_str((uint8_t*)"REFLOW", 6, 40, y, FONT6X6);   // ***************
		 
		// Display indexed parameters on LCD lines 2-7   // *******************************
		y += 7;  // **************************
		int j = line2Idx - cursorOffset; // *************
		for (int i = j; i  < (j + 6); i++) {  // *************************
			if(	(line2Idx == 2 && i == (line2Idx - cursorOffset + 3)) ||  // *******************
				(line2Idx == 3 && i == (line2Idx - cursorOffset + 2)) || // ******************
				(line2Idx == 4 && i == (line2Idx - cursorOffset + 1)) ) {   // ******************
				LCD_disp_str((uint8_t*)"SOAK", 4, 40, y, FONT6X6);  // ***************
				// --j;  // *********
				y+=7; // ****************************
			}  // ******************************
			else if( (line2Idx == 6 && i == (line2Idx - cursorOffset + 4)) ||  // ************************
					 (line2Idx == 8 && i == (line2Idx - cursorOffset + 2)) ||  // ************************
					 (line2Idx == 9 && i == (line2Idx - cursorOffset + 1)) ) {  // ************************
				LCD_disp_str((uint8_t*)"REFLOW", 6, 40, y, FONT6X6);  // ***************
				// --j;  // ***********
				y+=7; // ****************************
			}  // ************************
			else if( line2Idx == 14 && i == (line2Idx - cursorOffset + 3) ) {  // *****************************
				LCD_disp_str((uint8_t*)"COOLDOWN", 8, 40, y, FONT6X6);  // ***************
				// --j;  // *********
				y+=7; // ****************************
			} // ****************************
			// for two text line params  // ************************
				...
				(cursorPos == 8 && (  // *********************
					(line2Idx == 8 && i == (line2Idx - cursorOffset + 4)) || // *****************
					(line2Idx == 9 && i == (line2Idx - cursorOffset + 3)) || // *****************
					(line2Idx == 11 && i == (line2Idx - cursorOffset + 1))  // **************
				)) ||	// ***********************
				(cursorPos == 10 &&	(  // ****************
					(line2Idx == 11 && i == (line2Idx - cursorOffset + 4)) || // ******************
					(line2Idx == 14 && i == (line2Idx - cursorOffset + 3))  // ****************
				))  // *********************
				) {  // *******************
				...
				LCD_disp_str((uint8_t*)buf, count , 1, y, FONT6X6 | ( i == cursorPos ) ? INVERT : 0); // **********
				y +=7; // ****************************
				--j; // ****************
				count +=2; // set count for after '\n' // *************
				LCD_disp_str((uint8_t*)buf + count, len - count, 2, y, FONT6X6 | ( i == cursorPos ) ? INVERT : 0); // ***********
				y +=7; // ****************************
			} // *************************
			else {  // ***********************
				len = Reflow_editParamMenu_snprintFormattedValue(buf, sizeof(buf), i); // *************
				LCD_disp_str((uint8_t*)buf, len, 1, y, FONT6X6 | ( i  == cursorPos ) ? INVERT : 0); // **********
				y+=7; // ****************************
			} // ************
		}  // **********************
