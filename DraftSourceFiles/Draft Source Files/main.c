	...
	*/  // ****************************************
	//*  // ****************************************
		...
		if (keyspressed & KEY_F1) {	// move cursor to left or loop back  // *************
			cursorPos = (cursorPos - 1) - keyrepeataccel; // Move to next space left // ******************************
			if (cursorPos < 0) {  // ******************************************
				cursorPos = (PROFILE_NAME_LEN - 1) - keyrepeataccel;  // Wrap  // ******************************
			} // ****************************************
		}  // *************************************
		if (keyspressed & KEY_F2) { // move cursor to right or loop back  // ***************
			cursorPos = (cursorPos + 1) + keyrepeataccel; // Move to next space right // ******************
			if (cursorPos > (PROFILE_NAME_LEN - 1)) {  // *******************
				cursorPos = 0 + keyrepeataccel;	 // wrap  // ********************************************
			} // ********************************************
		}  // *******************************************
		if (keyspressed & KEY_F3) {  // ASCII values 32-96, & 123-126 are only available fonts  // *************************
			// Decrease char ASCII decimal value   // *********************
			int repeataccel = keyrepeataccel; // ********************
			editName[cursorPos] = (editName[cursorPos] - 1) - repeataccel; // Decrease ASCII decimal value // *********************
			if (editName[cursorPos] < 123 && editName[cursorPos] > 96)  // ******************
				editName[cursorPos] = 96 - repeataccel; // skip // *******************
			if ( editName[cursorPos] < 32 ) // *********************************
				editName[cursorPos] = 126 - repeataccel;  // wrap // ****************
		}  // ***************************************
		if (keyspressed & KEY_F4) {  // 32-96, & 123-126 are valid ASCII character values // **************************************
			// Increase char ASCII decimal value   // *********************
			int repeataccel = keyrepeataccel; // ********************
			editName[cursorPos] = (editName[cursorPos] + 1) + repeataccel; // Increase ASCII decimal value // *********************
			if (editName[cursorPos] > 96 && editName[cursorPos] < 123 ) // ** (wrap) ******************************
				editName[cursorPos] = 123 + repeataccel; //
			if (editName[cursorPos] > 126)  // ****************
				editName[cursorPos] = 32 + repeataccel; // wrap // ***********
		} // ***************************************
