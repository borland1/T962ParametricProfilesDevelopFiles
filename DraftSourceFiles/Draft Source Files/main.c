...
static int32_t Main_Work(void) {
	...
	//* / *******************************
		...
		y += 7;
		for (int i = lineIdx[1]; i < lineIdx[1] + 5; i++) { // ***************************
				...
				if (cursorPos > 11 && i == lineIdx[0] + lineIdx[2] ) {
					y+=7; // ****************************
		...
		if (keyspressed & KEY_S) {  // ******************************
			...
			lineCnt = 2; // Reset line counter // ********************
			Reflow_computeTempDatasetFromParams( ee4, ee3 ); // Update profile temp dataset // *********************
			Reflow_SaveProfileToEEPROM( ee3 ); // Save profile to EEPROM // *********************
	...
	/*   //  ****** this MAIN_EDIT_PROFILE code block to be moved  *************************  
	} else if (mode == MAIN_EDIT_PROFILE) {
		LCD_FB_Clear();
		int keyrepeataccel = keyspressed >> 17; // Divide the value by 2
		if (keyrepeataccel < 1) keyrepeataccel = 1;
	...
	*/  // end  MAIN_EDIT_PROFILE code block to be moved  // ***********************
