...
static int32_t Main_Work(void) {
	...
	if (mode == MAIN_SETUP) {
		...
		char phaseStr[10]; // *********************
		if (ticks < endTime.preheat) strcpy(phaseStr, "Preheat", 7); // *********************
		else if (ticks < endTime.soak) strcpy(phaseStr, "Soak", 4); // *********************
		else if (ticks < endTime.reflow) strcpy(phaseStr, "Reflow", 6); // *********************
		else strcpy(phaseStr, "Cooldown", 8); // *********************
		phaseStr[9] = '\0'; // *********************

		char scrollText[40]; // *********************
		len = snprintf(scrollText, sizeof(scrollText), "%s - %s", Reflow_GetProfileName(&ee3), phaseStr); // *********************
			Reflow_GetModeStr()); // *********************
		if (sched_get_time() - scrollTimer > TICKS_MS(300)) { // *********************
			scrollOffset++; // *********************
			if (scrollOffset >= strlen(scrollText)) scrollOffset = 0; // *********************
			scrollTimer = sched_get_time(); // *********************
		} // *********************
		char displayLine[21]; // *********************
		strncpy(displayLine, scrollText + scrollOffset, 20); // *********************
		displayLine[21] = '\0'; // *********************
		LCD_disp_str(displayLine, sizeof(displayLine), 13, 0, FONT6X6); // *********************
		}
		*/ // *********************
