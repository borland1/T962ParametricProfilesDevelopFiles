...
static int32_t Main_Work(void) {
	...
	if (mode == MAIN_SETUP) {
		...
		if (ticks > 5 && ( !Reflow_IsDone() ) ) { // wait 5 seconds before clearing line // *********************
			...
			//char statusLine[20]; // *********************
			LCD_disp_str((uint8_t*)currentPhase, strlen(currentPhase), LCD_ALIGN_RIGHT(strlen(currentPhase)), 0, FONT6X6); // *************
			// Display remaining time // *********************
			len = snprintf(buf, sizeof(buf), " %1u:%02u", (unsigned int) remaining/60, (unsigned int) (remaining % 60)); // *********************
		...
		len = snprintf(buf, sizeof(buf), "%s", Reflow_GetProfileName(&ee3)); // ****
		LCD_disp_str((uint8_t*)buf, len, 0, 8 * 1, FONT6X6 | INVERT); // *************
		...
		LCD_disp_str((uint8_t*)"RENAME PROFILE", 14, 14, 8 * 4, FONT6X6); // ********
		LCD_disp_str((uint8_t*)"F3", 2, 0, 8 * 5, FONT6X6 | INVERT); // ***********
		LCD_disp_str((uint8_t*)"RESTORE PROFILE", 15, 14, 8 * 5, FONT6X6); // ******
		LCD_disp_str((uint8_t*)"DEFAULTS", 8, 14, 8 * 6, FONT6X6); // ******
		LCD_disp_str((uint8_t*)"F4", 2, 0, 8 * 7, FONT6X6 | INVERT); // *****************
		LCD_disp_str((uint8_t*)"RETURN", 6, 14, 8 * 7, FONT6X6); // ***********
		...
		*/ // *****************************
		...
		LCD_disp_str((uint8_t*)"RESTORE PROFILE", 15, 0, 8 * 2, FONT6X6);  // **********
		LCD_disp_str((uint8_t*)"DEFAULTS?", 9, 0, 8 * 3, FONT6X6);  // **********
