...
#include "LPC214x.h"
#include <stdlib.h>  // malloc() and free() used // *************
#include <stdint.h>
#include <stdio.h>
#include <string.h>
#include "serial.h"
...
#define TEST_VERSION "PP10.10.2025"  // Parametric Profiles // *********************
...
"\n" \
"\nInitializing improved reflow oven..."; // fixed URL typo // *********************
...
int main(void) {
	...
	len = snprintf(buf, sizeof(buf), "FW: %s", TEST_VERSION);  // *********************
...
static int32_t Main_Work(void) {
	...
	if (mode == MAIN_SETUP) {
		...
		if (keyspressed & KEY_S) {
			...
			len = snprintf(buf, sizeof(buf), "FW: %s", TEST_VERSION); // *********************
			LCD_disp_str((uint8_t*)buf, len, 7, 64 - 6, FONT6X6); // *************
		...
		uint16_t ticks = (uint16_t) RTC_Read();
		
		if (ticks > 5 && ( !Reflow_IsDone() ) ) { // wait 5 seconds before clearing line // *********************
			for(int i = 0; i < PROFILE_NAME_LEN; i++)  // *********************
				LCD_disp_str((uint8_t*)" ", 1, 13 + (i * 6), 0, FONT6X6); // Clear line // *********************
			
			uint16_t remaining; // *********************
			if (ticks < endTime.preheat) {  // *********************
				remaining = endTime.preheat - ticks; // **********
				LCD_disp_str((uint8_t*)"PREHEAT", 7, LCD_ALIGN_RIGHT(7), 0, FONT6X6); // *************
			} else if (ticks < endTime.soak) {  // *********************
				remaining = endTime.soak - ticks;  // *******************
				LCD_disp_str((uint8_t*)"SOAK", 4, LCD_ALIGN_RIGHT(4), 0, FONT6X6); // *************
			} else if (ticks < endTime.reflow) {  // *********************
				remaining = endTime.reflow - ticks;  // *****************
				LCD_disp_str((uint8_t*)"REFLOW", 6, LCD_ALIGN_RIGHT(6), 0, FONT6X6); // *************
			} else {  			// ********************* 
				remaining = endTime.cooldown - ticks; // **********
				LCD_disp_str((uint8_t*)"COOLDOWN", 8, LCD_ALIGN_RIGHT(8), 0, FONT6X6); // *************
			}; // *********************
			
			// Display time remaining  //  **********************
			len = snprintf(buf, sizeof(buf), "%1u", (unsigned int) (remaining / 60));  // *************
			LCD_disp_str((uint8_t*)buf, (uint8_t)len, 13, 0, FONT6X6);  // *************
			len = snprintf(buf, sizeof(buf), "%02u", (unsigned int) (remaining % 60 ));  // ************** 
		...
		if( !Reflow_IsDone() ) {
			...
			len = snprintf(buf, sizeof(buf), "%1u",  // *************
				(unsigned int) (((uint16_t) endTime.cooldown - ticks ) / 60));  // *************
			LCD_disp_str((uint8_t*)buf, len, 108, 39, FONT6X6);  // *************
			len = snprintf(buf, sizeof(buf), "%02u",  // *************
				(unsigned int) (((uint16_t) endTime.cooldown - ticks) % 60));  // ************** 
		...
		/**/   // *********************
		if ( Reflow_IsDone() ) {  // *********************
			static uint8_t beeped = 0;  // *********************
			
			Reflow_SetMode(REFLOW_STANDBY);  // *************
			// cycle thru messages every 2 seconds  // *********************
			if (ticks % 7 == 2) { // ***********
				...
				for(int i = 0; i < PROFILE_NAME_LEN; i++)  // *********************
					LCD_disp_str((uint8_t*)" ", 1, 13 + (i * 6), 0, FONT6X6); // Clear line // *********************
				LCD_disp_str((uint8_t*)"PROFILE COMPLETED", 17, 13, 0, FONT6X6); // *************
			} else if (ticks % 7 == 6) {  // *************
				LCD_disp_str((uint8_t*)"   RESTART,   EXIT", 18, 13, 0, FONT6X6); // Clear previous phase // *************
				LCD_disp_str((uint8_t*)"F1", 2, 13, 0, FONT6X6 | INVERT); // *************
				LCD_disp_str((uint8_t*)"S,", 1, 13 + (12 * 6), 0, FONT6X6 | INVERT); // *************
			} // *************
			if ( !beeped ) {  // *************
				Buzzer_Beep(BUZZ_1KHZ, 255, TICKS_MS(100) * NV_GetConfig(REFLOW_BEEP_DONE_LEN)); // *************
				printf("\nReflow done\n"); // *************
				beeped = 1; // only beep once // *************
			} // *************
			if ( keyspressed & KEY_F1) { // *************
				beeped = 0; // reset for next time // *************
				LCD_FB_Clear(); // *************
		...
		if (Reflow_IsDone() || keyspressed & KEY_S) {  // *************
			// printf("\nReflow %s\n", (Reflow_IsDone() ? "done" : "interrupted by keypress"));  // moved below // *************
			if (Reflow_IsDone()) {  // *************
				Buzzer_Beep(BUZZ_1KHZ, 255, TICKS_MS(100) * NV_GetConfig(REFLOW_BEEP_DONE_LEN)); // *************
		...
		menu_idx* idx; // *****************
		if (idx == NULL) {  // ******************
			idx = (menu_idx*)malloc(sizeof(menu_idx)); // ****
			idx->cursor_pos = 0;  // *****************
			idx->line1_idx = 1; // index for display line two, vertical scrolling // *********************
			idx->line2_idx = 2; // index for display line three, vertical scrolling // *********************
		}  // ******************
		int8_t cursorPos = idx->cursor_pos; // *****************
		int8_t line1Idx = idx->line1_idx; // index for display line two, vertical scrolling // *********************
		int8_t line2Idx = idx->line2_idx; // index for display line
		*/  // *********************
		...
		if (line1Idx == 1 )       // ******************************
			...
			LCD_disp_str((uint8_t*)"-SOAK-", 6, (LCD_ALIGN_CENTER(6) - 1), y, FONT6X6);     // ***************
		else if (line1Idx == 10)  // ***************************
		...
		for (int i = line2Idx; i  < line2Idx + 6; i++) {  // i = line2Idx + # inc.s = line # // *************************
			...
			// for two text line params  // ************************
				...
				LCD_disp_str((uint8_t*)bigBuf, count , 2, y, FONT6X6 | ( (i - cursorOffset) == cursorPos ) ? INVERT : 0); // **********
				y +=7; // ****************************
				++count; // set count for after '\n' // *************
				LCD_disp_str((uint8_t*)bigBuf + count, len - count, 2, y, FONT6X6 | ( (i - cursorOffset) == cursorPos ) ? INVERT : 0); // ***********
				y +=7; // ****************************
				++i;  // *********************
			} // *************************
			else {  // ***********************
				len = Reflow_editParamMenu_snprintFormattedValue(buf, sizeof(buf), i - cursorOffset); // *************
				LCD_disp_str((uint8_t*)buf, len, 2, y, FONT6X6 | ( (i - cursorOffset)  == cursorPos ) ? INVERT : 0); // **********
				y+=7; // ****************************
			} // ************
		}  // **********************
		// plot vertical scroll bar graphic // *******************************
		LCD_BMPDisplay(scrollBarTicksbmp, 127, 2); // *************
		LCD_BMPDisplay(scrollBarbmp, 127, 3 + (cursorPos * 4) ); // *************
		...
		/* // *********************
		idx->cursor_pos = cursorPos; // save for next loop // *****************
		idx->line1_idx = line1Idx; // *********************
		idx->line2_idx = line2Idx; // *********************
		*/  // *********************		
		...
		edit_name_menu* enm; // *********************
		if (enm == NULL) {  // *********************
			enm = (edit_name_menu*)malloc(sizeof(edit_name_menu)); // *********************
			enm->cursor_pos = 0; // *********************
			enm->edit_name = (char*)malloc(PROFILE_NAME_LEN * sizeof(char)); // *********************
			strncpy(enm->edit_name, Reflow_GetProfileName(&ee3), PROFILE_NAME_LEN); // *********************
		}  // *********************
		int8_t cursorPos = enm->cursor_pos; // *********************
		char editName[PROFILE_NAME_LEN];  // ********************* 
		strncpy(editName, enm->edit_name, PROFILE_NAME_LEN);  // *********************
		*/  // *********************
		
		static char editName[PROFILE_NAME_LEN]; // *********************
		static int8_t cursorPos = 0; // Profile Name Cursor Position // *********************
		static uint8_t initialized = 0; // *********************
		
		if (initialized == 0) {  // *********************
			strncpy(editName, Reflow_GetProfileName(&ee3), PROFILE_NAME_LEN); // *********************
			initialized = 1; // *********************
		}  // *********************
		...
		for ( int i = 0; i < PROFILE_NAME_LEN; i++) {  // *******************
			...
			// this for loop just adds 1x7 pixels INVERT effect on right side of selected char // *******************
			for (int j = 0; j < 8; j++) {  // **********************
		...
		/*  // *********************
		enm->cursor_pos = cursorPos; // save for next loop // *********************
		strncpy(enm->edit_name, editName, PROFILE_NAME_LEN); // *********************
		*/  // *********************
		...
		if (keyspressed & KEY_S) {  // same renamed profile to RAM and EEPROM // ****************************
			...
			/*	// *********************
			// free(editName); // free malloc'd memory // *********************
			// free(enm->edit_name); // free malloc'd memory // *********************
			free(enm); // free malloc'd memory // *********************
			enm = NULL; // *********************
			*/  // *********************
	...
	/*   // this MAIN_EDIT_PROFILE code block, is replaced with above menus // *************************  
	} else if (mode == MAIN_EDIT_PROFILE) {
		LCD_FB_Clear();
		int keyrepeataccel = keyspressed >> 17; // Divide the value by 2
		if (keyrepeataccel < 1) keyrepeataccel = 1;
	...
	*/  // -end  this MAIN_EDIT_PROFILE code block, is replaced with above menus // ***********************
