...
static int32_t Main_Work(void) {
	...
	if (mode == MAIN_SETUP) {
		...
		for (int i = line2Idx - cursorOffset; i < line2Idx + 6; i++) {  // *************************
			...
			if(  
				...
				(cursorPos == 10 &&	(
					...
					(line2Idx == 14 && i == (line2Idx + 3))
				))
				) {  
				// for two line menu params // *************
				int count = 0; // *************
				for (int j = 0; buf[j] != '\n'; j++) {  // find count before'\n' *************
					if (buf[j] != '\n') count++;  // *************
				} // ****************************
			...
			else { LCD_disp_str((uint8_t*)buf, len, 1, y, FONT6X6 | ( i == cursorPos ) ? INVERT : 0); } // ************
			y+=7; // ****************************
		}  // **********************
