...
static int32_t Main_Work(void) {
	...
	if (mode == MAIN_SETUP) {
		...
		if (keyspressed & KEY_F1) {	// Previous, vertical scrolling // *************
			--cursorPos;  // *********************
			// Adjust cursorOffset // *****************
			if (cursorPos == 2) cursorOffset = 2;  // *************
			else if(cursorPos == 6) cursorOffset = 3; // ************
			else if(cursorPos == 7) cursorOffset = 4;  // ************
			else if(cursorPos == 9) cursorOffset = 5;  // ***********
			else if(cursorPos == 10) cursorOffset = 6;  // *************
			else if(cursorPos < 0) cursorOffset = 2;  // ************
		...
		if (keyspressed & KEY_F2) {	// Next, vertical scrolling // *************
			++cursorPos;  // *******************
			// Adjust cursorOffset  // **************
			if (cursorPos == 3) cursorOffset = 3;  // *************
			else if(cursorPos == 7) cursorOffset = 4; // ************
			else if(cursorPos == 8) cursorOffset = 5;  // ************
			else if(cursorPos == 10) cursorOffset = 6;  // ***********
			else if(cursorPos == 11) cursorOffset = 7;  // *************
			else if(cursorPos > 12) cursorOffset = 2;  // ************
		...
		for (int i = line2Idx; i  < line2Idx + 6; i++) {  // *************************
			...
			// for two text line params  // ************************
				...
				LCD_disp_str((uint8_t*)buf, count , 1, y, FONT6X6 | ( (i - cursorOffset) == cursorPos ) ? INVERT : 0); // **********
				y +=7; // ****************************
				--i; // ****************
				count +=2; // set count for after '\n' // *************
				LCD_disp_str((uint8_t*)buf + count, len - count, 2, y, FONT6X6 | ( (i - cursorOffset) == cursorPos ) ? INVERT : 0); // ***********
				y +=7; // ****************************
			} // *************************
			else {  // ***********************
				len = Reflow_editParamMenu_snprintFormattedValue(buf, sizeof(buf), i); // *************
				LCD_disp_str((uint8_t*)buf, len, 1, y, FONT6X6 | ( (i - cursorOffset)  == cursorPos ) ? INVERT : 0); // **********
				y+=7; // ****************************
			} // ************
		}  // **********************
