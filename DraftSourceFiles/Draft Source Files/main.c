...
static int32_t Main_Work(void) {
	...
	if (mode == MAIN_SETUP) {
		...
		static int8_t cursorPos = 1;  // cursor at parameter value position // *************************
		...
		if (keyspressed & KEY_F1) {	// Previous, vertical scrolling // *************
			--cursorPos;  // *************
			if ( ( cursorPos == 1 && line2Idx == 3 ) ||   // ***************
			     ( cursorPos == 2 && line2Idx == 4 ) ||  // ***************
			     ( cursorPos == 6 && line2Idx == 9 ) ) { // ***************
 					--line2Idx;  // ***************
			}  // ***************
			if ( cursorPos == 3 && line2Idx == 6 ) { // *****************
				  	line1Idx -= 4;  // ***************
				  	line2Idx -= 2;  // *************** 
			}  // ***************
			if ( cursorPos == 5 && line2Idx == 8 ) { // ***************
					line2Idx -= 2;  // *************** 
			}  // ***************
			if ( cursorPos == 7 && line2Idx == 11 ) { // *****************
				  	line1Idx -= 5;  // ***************
				  	line2Idx -= 2;  // *************** 
			}  // ***************
			if ( cursorPos == 9 && line2Idx == 14 ) { // ***************
				  	line2Idx -= 3;  // *************** 					
			}  // ****************
			if ( cursorPos < 1 ) { // wrap around // ****************
				 cursorPos = 13;  // *****************
				 line1Idx = 10;  // *********************
				 line2Idx = 14;  // *******************
			}    // ******************
		...
		if (keyspressed & KEY_F2) {	// Next, vertical scrolling // *************
			++cursorPos;  // *************
			if ( ( cursorPos == 6 && line2Idx == 2 ) ||   // ***************
			     ( cursorPos == 7 && line2Idx == 3 ) ||  // ***************
			     ( cursorPos == 10 && line2Idx == 8 ) ) { // ***************
 					++line2Idx;  // ***************
			}  // ***************
			if ( cursorPos == 8 && line2Idx == 4 ) { // *****************
				  	line1Idx += 4;  // ***************
				  	line2Idx += 2;  // *************** 
			}  // ***************
			if ( cursorPos == 9 && line2Idx == 6 ) { // ***************
					line2Idx += 2;  // *************** 
			}  // ***************
			if ( cursorPos == 11 && line2Idx == 9 ) { // *****************
				  	line1Idx += 5;  // ***************
				  	line2Idx += 2;  // *************** 
			}  // ***************
			if ( cursorPos == 12 && line2Idx == 11 ) { // ***************
				  	line2Idx += 3;  // *************** 					
			}  // ****************
			if ( cursorPos > 13 ) { // wrap around // ****************
				 cursorPos = 1;  // *****************
				 line1Idx = 1;  // *********************
				 line2Idx = 2;  // *******************
			}    // ******************
		...
		if (line1Idx == 1 )  // ******************************
			LCD_disp_str((uint8_t*)"-- PREHEAT --", 7 + 6, LCD_ALIGN_CENTER(7 + 6), y, FONT6X6);  // ***************
		else if (line1Idx == 5) // ***************************
			LCD_disp_str((uint8_t*)"-- SOAK --", 4 + 6, LCD_ALIGN_CENTER(4 + 6), y, FONT6X6);     // ***************
		else if (line1Idx == 10) // ***************************
			LCD_disp_str((uint8_t*)"-- REFLOW --", 6 + 6, LCD_ALIGN_CENTER(6 + 6), y, FONT6X6);   // ***************
		else if (line1Idx == 17) // (lineIdx == 17) // ***************************
			LCD_disp_str((uint8_t*)"-- COOLDOWN --", 8 + 6, LCD_ALIGN_CENTER(8 + 6), y, FONT6X6);   // ***************
		y += 7;  // **************************
		
		// Display indexed parameters on LCD lines 2-7 // *******************************
		for (int i = line2Idx; i < line2Idx + 7; i++) { // ***************************
			...
			if (cursorPos < 4 && i == (line2Idx + 3)) { // *****************************
				LCD_disp_str((uint8_t*)"-- SOAK --", 4 + 6, LCD_ALIGN_CENTER(4 + 6), y, FONT6X6);  // ***************
				y+=7; // ****************************
				++i; // ***************
			}
			else if (cursorPos > 3 && cursorPos < 8 && i == (line2Idx + 5)) {  // *****************************
				LCD_disp_str((uint8_t*)"-- REFLOW --", 6 + 6, LCD_ALIGN_CENTER(6 + 6), y, FONT6X6);  // ***************
				y+=7; // ****************************
				++i; // ***************
			}
			else if (cursorPos > 11 && i == (line2Idx + 3)) {  // *****************************
				LCD_disp_str((uint8_t*)"-- PREHEAT --", 7 + 6 , LCD_ALIGN_CENTER(7 + 6), y, FONT6X6);  // ***************
				y+=7; // ****************************
				++i; // ***************	
			}
