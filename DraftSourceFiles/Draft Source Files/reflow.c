...
#define PMAGIC (('P' << 8) | 'J') // Check for Parameter Profiles installed? // **********
// #define PMAGIC (('Q' << 8) | 'J') // Check for Parameter Profiles installed? // **********
...
void Reflow_Init(void) {
	...
	/* Reflow_LoadCustomProfiles(); */ // not used, replaced with below  // *************
	
	Reflow_ValidateNV();

	// Check if parameter profiles are initialized in EEPROM // *************
	uint16_t pmagic = 0; // pmagic number for parameter profiles // *************
	EEPROM_Read((uint8_t*) &pmagic, (uint32_t) 0x00, (uint32_t) sizeof(pmagic)); // *************
	if (pmagic != PMAGIC) { // **************
		printf("\nNo param profiles found, initializing default pram profiles in EEPROM."); // *************
		for (int i = 0; i < NUM_PARAMETER_PROFILES; i++) { // *************
			uint8_t addr = 0; // **************
			switch(i) { // ********************
			...
			Reflow_pack_profile(&paramProfiles[i - 1], packBuffer); // **************
			EEPROM_Write( (uint32_t) addr, (uint8_t*) packBuffer, (uint32_t) sizeof(packBuffer) ); // **********
		}  // **************
		uint16_t pmagicBuffer = PMAGIC; // **************
		EEPROM_Write( (uint32_t) 0x00, (uint8_t*) &pmagicBuffer, (uint32_t) sizeof(pmagicBuffer)); // **********
	}
	// Read packed Profile Parameters from EEPROM, unpack, compute & setup profile temps dataset in RAM // *********
