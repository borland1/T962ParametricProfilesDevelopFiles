...
void Reflow_Init(void) {
	...
	uint16_t* pmagic = 0; // Pointer to magic number for parameter profiles // *************
	EEPROM_Read((uint8_t*)&pmagic, 0x00, sizeof(pmagic)); // *************
	if (pmagic != PMAGIC) { // **************
		...
		for (int i = 0; i < NUM_PARAMETER_PROFILES; i++) { // *************
			...
			pack_profile(paramProfiles[i], *packBuffer); // **************
			EEPROM_Write( addr, (uint8_t*) &packBuffer, sizeof(packBuffer) ); // **********
		}  // **************
		uint16_t pmagicBuffer = PMAGIC; // **************
		EEPROM_Write( 0x00, (uint8_t*) &pmagicBuffer, sizeof(pmagicBuffer)); // **********
	} // **************
